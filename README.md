...
1. Update package.json:
   ```json
   {
     "name": "@yourscope/claude-skill-pairl",
     "version": "1.1.0",
     "description": "Claude Code skill for PAIRL v1.1",
     "main": "skill.json",
     "files": ["skill.md", "skill.json", "README.md"],
     "keywords": ["claude-code-skill", "pairl", "agent-protocol"]
   }
   ```
...
1. Create repository: `PAIRL`
2. Push skill to `claude-skill/` directory
3. Tag release: `git tag -a v1.1.0 -m "PAIRL v1.1 skill"`
...
Follow the [Claude Code Skill Registry guidelines](https://docs.anthropic.com/claude-code/skills/registry) to submit your skill for inclusion in the official registry.
...
2. Test in Claude Code session:
   ```
   /pairl generate: Test message
   ```

3. View skill logs:
   ```bash
   claude-code skill logs pairl
   ```

### Updating the Skill

1. Make changes to `skill.md` or `skill.json`
2. Increment version in `skill.json`
3. Reinstall:
   ```bash
   claude-code skill update pairl
   ```
